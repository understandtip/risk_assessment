...
import com.yushang.risk.common.constant.RedisConstant;
...
import com.yushang.risk.common.util.VerifyCodeUtil;
...
public class UsersServiceImpl implements UsersService {
  ...
  public Object[] getCode(String sessionId) {
    // 把旧的验证码删除,生成新的返回
    RedisUtils.del(RedisConstant.USER_REDIS_KEY_PREFIX + sessionId);
    ...
    log.info("code:{}", code);
    RedisUtils.set(
        RedisConstant.USER_REDIS_KEY_PREFIX + sessionId,
        code,
  ...
  public LoginUserResp login(LoginReq loginReq, HttpSession session) {
    ...
    return UserAdapter.buildLoginUserResp(user);
  }
  ...
  private void verifyCode(String code, String sessionId) {
    String s = RedisUtils.getStr(RedisConstant.USER_REDIS_KEY_PREFIX + sessionId);
